...
class ModelArguments:
    ...
    use_lora: bool = field(
        default=False,
        metadata={"help": "Whether to use lora or full-parameter tunning"}
    )
    n_bits: int = field(
        default=32,
        metadata={
            "help": "Whether to use 32/16 bit for base model weights or 4 or 8 bits(QLoRA)"
        }
    )
...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        input_ids = p.inputs
        input_ids = [
            [token_id for token_id in vec if token_id != -100]
            for vec in input_ids
        ]
        language_pairs = np.array([
            x.split("\n\n")[0].replace("Language-pair: ", "") 
            for x in tokenizer.batch_decode(
                input_ids,
                skip_special_tokens=True
            )
        ])
        unique_lps = set(language_pairs)
        result = {}
        for lp in unique_lps:
            mask = language_pairs == lp
            kt = kendalltau(preds[mask], scores[mask]).statistic
            result[f'kendalltau|{lp}'] = kt
            
        result['kendalltau|total'] = kendalltau(preds, scores).statistic

#!/bin/bash
#SBATCH --job-name=synthplus-mdeberta-no-freeze
#SBATCH --output=./logs/synthplus-mdeberta-no-freeze-%A-[%a].txt
#SBATCH --nodes=1
...
WANDB_NAME=synthplus-mdeberta-no-freeze-${SLURM_ARRAY_TASK_ID} \
WANDB_RUN_GROUP="synthplus-mdeberta-no-freeze" \
python train.py \
    --seed=${SLURM_ARRAY_TASK_ID} \
    --output="distillation_results/synthplus-mdeberta-no-freeze-${SLURM_ARRAY_TASK_ID}" \
